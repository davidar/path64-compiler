...
set(_PATH64_SUPPORTED_TARGETS_STRING
...
set(_PATH64_ARCH_FLAGS_arm
	"-DTARG_ST -DTARG_ARM -DBE_EXPORTED= -DTARGINFO_EXPORTED= -DSUPPORTS_SELECT -DMUMBLE_ARM_BSP")	# TODO: fix it
...
foreach(targ ${PATH64_ENABLE_TARGETS})
	...
	if(NOT targ_arch)
		message(FATAL_ERROR
			"'${targ}' is not among supported architectures.
...
function(path64_check_arch_exists aname)
	if(NOT _PATH64_ARCH_FLAGS_${aname})
		message(FATAL_ERROR
			"Architecture with name '${aname}' does not exist")
	endif()
endfunction()
...
option(PATH64_USE_SYSTEM_COMPILER_FOR_TARGET_LIBS
	"Use system compiler for building target libraries"
	OFF)
...
function(path64_add_library_for_target name target type src_base_path)
	...
	set(arch_flag
		${_PATH64_TARGET_FLAG_${target}}
		${_PATH64_ARCH_FLAGS_${arch}})
...
#    set_property(TARGET ${name} PROPERTY ARCHIVE_OUTPUT_DIRECTORY ${build_lib_dir})
	...
	set(compiler-deps)
	set(use_sys_compiler
		${path64_use_system_compiler_for_multitarget_${name}})
	...
	foreach(src ${sources})
		...
		if(NOT "${src_ext}" STREQUAL "h")
			...
			get_property(src_compile_defs_list
				SOURCE
				${src}
				PROPERTY
				COMPILE_DEFINITIONS)
			...
			foreach(def
				${path64_multitarget_property_${name}_COMPILE_DEFINITIONS})
			...
			foreach(dep ${obj_depends_rel})
				list(APPEND
					obj_depends
					${CMAKE_CURRENT_BINARY_DIR}/${name}-${targ}/${dep})
			endforeach()
			...
			foreach(out ${obj_outputs_rel})
				list(APPEND
					obj_outputs
					${CMAKE_CURRENT_BINARY_DIR}/${name}-${targ}/${out})
			endforeach()
			...
			set(object_name
				"${CMAKE_CURRENT_BINARY_DIR}/${name}-${targ}/${object_rel_name}")
			...
			path64_get_compiler_for_language(compiler
				"${src_lang}"
				"${use_sys_compiler}")
			...
			add_custom_command(OUTPUT
				...
				${obj_depends}
				WORKING_DIRECTORY
				${CMAKE_CURRENT_BINARY_DIR}/${name}-${targ})
	...
	elseif("X${type}" STREQUAL "XSHARED")
		string(REPLACE
			...
			";"
			target_link_flags
			"${path64_multitarget_property_${name}_LINK_FLAGS}")
		set(library_file
	...
	add_custom_target(${name}-${targ} ALL DEPENDS ${library_file})
	add_dependencies(${name}-${targ} ${compiler-deps})

	install(FILES ${library_file} DESTINATION ${install_lib_dir})
...
function(path64_add_multitarget_library name type)
	...
	foreach(targ ${PATH64_ENABLE_TARGETS})
		if(${src_list_name}_${targ})
			path64_add_library_for_target(${name}
				${targ}
				${type}
				"${src_base_path}"
...
function(path64_add_multiarch_library name type src_list_name)
	foreach(arch ${PATH64_ENABLE_ARCHES})
		...
		if(${src_list_name}_${arch})
			path64_add_library_for_arch(${tg_name}
				${arch}
				${type}
				${${src_list_name}_${arch}})
		else()
			path64_add_library_for_arch(${tg_name}
				${arch}
				${type}
...
function(path64_add_multiarch_executable name src_list_name)
	foreach(arch ${PATH64_ENABLE_ARCHES})
		path64_get_multiarch_cmake_target(tg_name ${name} ${arch})
		if(${src_list_name}_${arch})
			path64_add_executable_for_arch(${tg_name}
				${arch}
				${${src_list_name}_${arch}})
		else()
			path64_add_executable_for_arch(${tg_name}
...
function(path64_set_property_for_target name targ prop)
	...
	if(prop MATCHES "COMPILE_FLAGS" OR prop MATCHES "LINK_FLAGS")
		...
		set(arch_flag
			"${_PATH64_TARGET_FLAG_${targ}} ${_PATH64_ARCH_FLAGS_${arch}}")
...
function(path64_set_multitarget_property_for_target name targ prop)
	set(path64_multitarget_property_${name}_${targ}_${prop}
		${ARGN}
		PARENT_SCOPE)
endfunction()
...
function(path64_add_multiarch_custom_target name)
	...
	foreach(arch ${PATH64_ENABLE_ARCHES})
		...
		if(arch_cmd_with_args)
			...
			add_custom_target(${tg} DEPENDS ${arch_depends})
		endif()
	endforeach()
endfunction()
...
function(path64_set_use_system_compiler_for_multitarget name)
	set(path64_use_system_compiler_for_multitarget_${name}
		TRUE
		PARENT_SCOPE)
endfunction()
